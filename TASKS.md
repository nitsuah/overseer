...
- [x] Stale-review detector for PR readiness.
  - Priority: P2
  ...
  - Status: ✅ SHIPPED — the `reviewDecision === CHANGES_REQUESTED` + all-threads-resolved + CI-green classification (`stale_review_count`) already existed from an earlier pass; this branch found and extended it rather than rebuilding it. Extracted the inline classification into a pure, independently-tested `isStaleReview()` (`lib/github/prs.ts`) and added `getStaleReviewPrNumbers()` so the badge can link straight to the (lowest-numbered) affected PR instead of the repo's generic PR list — plumbed end-to-end through `stale_review_pr_numbers` (new JSONB column, `lib/schema-migrations.ts`), `syncRepo` (`lib/sync.ts`), the `Repo` type (`lib/db.ts`/`types/repo.ts`), and both `RepoTableRow.tsx`/`MobileRepoCard.tsx` badges. 8 new tests for the two pure functions (`lib/github.test.ts`) plus updated integration coverage for the new field. Observed the exact gap this closes on 6 different PRs across the portfolio in one session (overseer #211, agent-board #73, darkmoon #448, bb-mcp #123, vhs #54) — each had to be found by hand via `gh pr view --json`.
...
- [ ] Reserve shared-key quota before a personal-key fallback call, not after.
  - Priority: P2
