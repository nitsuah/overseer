/**
 * Idempotent schema-evolution statements applied before every sync via ensureSchema().
 *
 * RULE: when you add a DB column or index, append a statement here.
 * Never create a new scripts/migrate-*.ts or one-off migration API route.
 */
export const SCHEMA_MIGRATIONS: readonly string[] = [
    ...
    // repos: PR readiness breakdown (these are the columns currently crashing prod sync)
